...
# Hint: use the ord for separately for the two characters in the parameter.
...
def two_character_to_int(two_character_string):
    ...
    '0x7e20'
    '''
    if len(two_character_string) != 2:
        raise Exception(f'The input string should be length of 2 for "two_character_to_int" function, input was:"{two_character_string}"')

    return int(hex(ord(two_character_string[0]))[2:] + hex(ord(two_character_string[1]))[2:],16)

    #return ord(two_character_string[0]) * 256 + ord(two_character_string[1])

# Create a function that converts an integer, that can be at most 2 byte sized, to ascii string.
...
def two_byte_int_to_string(two_byte_int):
    ...
    '~ '
    '''
    if two_byte_int >= 2**16:
        raise Exception(f'The input integer should be fit into 2 bytes input was:"{two_byte_int}"')

    hex_form = hex(two_byte_int)[2:].rjust(4, '0')
    return chr(int(hex_form[0:2],16)) + chr(int(hex_form[2:4],16))
...
# If the message is odd length, then pad it from the right with zero, and cut the cipher to the original length.
...
def encpryt_xor_by_two_byte_sized_key(message, key):
    ...
    for i in range(0,len(message),2):
        block_str = message[i:i+2]
        block_int = two_character_to_int(block_str)
        encrypted_block = block_int ^ key
        ret += two_byte_int_to_string(encrypted_block)

    return ret[0:original_length]


# Create an improved version of the previous function by change the key block by block by powering and modulo by 2**16.
# The first block (2 byte) will be encrypted similarly, but the second block's key will be calculated by the first block's key.
def encpryt_xor_by_two_byte_sized_key_changing_key(message, key):
    ...
    for i in range(0,len(message),2):
        block_str = message[i:i+2]
        block_int = two_character_to_int(block_str)
        encrypted_block = block_int ^ actual_key
        ret += two_byte_int_to_string(encrypted_block)
        actual_key = (actual_key ** 2) % (2**16)
